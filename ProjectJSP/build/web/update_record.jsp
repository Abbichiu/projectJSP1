    ...
    try {
        ...
        if (loginResult.next()) {
            ...
            if (studentResult.next()) {
                ...
                double gpaDouble = studentResult.getDouble("gpa");
                String gpa = String.valueOf(gpaDouble);

               double  cgpaDouble = studentResult.getDouble("cgpa");
               String cgpa = String.valueOf(cgpaDouble);
                ...
                request.setAttribute("gpa", Double.parseDouble(gpa));
                request.setAttribute("cgpa", Double.parseDouble(cgpa));
                ...
                errorMessage = "No student record found for the given username";
            }
        } else {
...
<body>
    ...
    String studid  = ( String)request.getAttribute("studid");
...
<% } %>
     <p><%= message %></p>
    <form  id="updateForm" action="update_record_action.jsp?username=<%= studid %>" method="POST">
    <!-- Student Information -->
...
<input type="number" id="gpa" name="gpa" step="0.1" min="0" max="4" value="<%= request.getAttribute("gpa") %>" disabled>
<button type="button" onclick="enableEdit('gpa')">Modify</button>

<label for="cgpa">CGPA:</label>
<input type="number" id="cgpa" name="cgpa" step="0.1" min="0" max="4" value="<%= request.getAttribute("cgpa") %>" disabled>
